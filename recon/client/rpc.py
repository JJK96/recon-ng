"""
Async RPC Client for the CLI to communicate with the RPC server.

This client uses aio-pika for async RabbitMQ communication and supports
real-time event streaming for interactive CLI output.
"""
import asyncio
import json
import logging
...
class CLIRPCClient:
    """
    Async RPC client for the CLI.
    
    Features:
    - Async request/response pattern
    - Real-time event streaming for interactive output
    - Connection pooling and auto-reconnect
    - Event handlers for different output types
    
    Usage:
        client = CLIRPCClient(amqp_url)
    ...
    def __init__(self, amqp_url: str = None):
        ...
        self._event_callbacks: Dict[str, Callable[[RPCEvent], Any]] = {}
    ...
    def connected(self) -> bool:
        return self._connected and self._connection and not self._connection.is_closed
    ...
    async def connect(self):
        ...
        try:
            logger.debug(f"Connecting to RabbitMQ at {self.amqp_url}")
            ...
            logger.debug(f"Connected as client {self.client_id}")
            
        except Exception as e:
            raise RPCConnectionError(f"Failed to connect to RabbitMQ: {e}") from e
    ...
    async def call(
        ...
        timeout: float = 30.0,
        event_callback: Callable[[RPCEvent], Any] = None
    ) -> Dict[str, Any]:
        """
